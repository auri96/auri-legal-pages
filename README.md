# Auri - Páginas Legales

Documentación legal de Auri alojada en GitHub Pages.

## Contenido

- **Privacy Policy**: Política de Tratamiento de Datos Personales (Ley 1581/2012)
- **Terms of Service**: Términos y Condiciones de Uso
- **Support**: Centro de Ayuda y Soporte

## URLs

- https://auri.com/privacy
- https://auri.com/terms
- https://auri.com/support

## Tecnología

- HTML5 semántico
- CSS3 (custom properties, flexbox, grid)
- Mobile-first responsive design
- GitHub Pages hosting

## Mantenimiento

**Última actualización:** 2026-04-14  
**Versión:** 1.0  
**Responsable:** Fintech Product Owner

Para actualizar el contenido legal, modificar los archivos fuente en:
`/docs/legal/` del repositorio principal de Auri.

## Cumplimiento Legal

- ✅ Ley 1581/2012 (Protección de Datos - Colombia)
- ✅ Decreto 1377/2013 (Reglamentación Habeas Data)
- ✅ Ley 1480/2011 (Estatuto del Consumidor)
- ✅ Google Play Developer Policy
- ✅ Apple App Store Guidelines
